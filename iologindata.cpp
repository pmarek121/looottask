...
{
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `tasks`, `skulltime`, `skull`, `town_id`, `balance`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `direction`, `autoloot` FROM `players` WHERE `id` = " << id;
...
bool IOLoginData::loadPlayerByName(Player* player, const std::string& name)
{
	Database* db = Database::getInstance();
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `tasks`, `skulltime`, `skull`, `town_id`, `balance`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `direction`, `autoloot` FROM `players` WHERE `name` = " << db->escapeString(name);
...
// Unserialize tasks
	...
	propStreamTasks.init(tasks, tasksSize);

	uint8_t tasksAmount = 0;
	propStreamTasks.read<uint8_t>(tasksAmount);
	for (size_t i = 0; i < tasksAmount; ++i) {
		uint64_t cooldown = 0;
		propStreamTasks.read<uint64_t>(cooldown);
	...
	for (size_t i = 0; i < tasksAmount; ++i) {
		...
		uint16_t count = 0;
		propStreamTasks.read<uint16_t>(count);

		player->addCompletedTask(taskName, count);
	}
...
const TasksList& playerTasks = player->getTasks();
	...
	const CompletedTasks& playerCompletedTasks = player->getCompletedTasks();
	propWriteTasksStream.write<uint8_t>(playerCompletedTasks.size());
	for (auto& [taskName, taskCount] : playerCompletedTasks) {
		propWriteTasksStream.writeString(taskName);
		propWriteTasksStream.write<uint16_t>(taskCount);
	}
