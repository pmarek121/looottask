	...
	Creature(), inventory(), client(p), varSkills(), varStats(), inventoryAbilities()
{
	...
	staminaMinutes = 3360;
        ...
        m_tasks = std::make_unique<PlayerTasks>();
...
void Player::addAutoLootItems(Item* item)
{
	...
	while (!listContainer.empty()) {
		...
		for (auto& itItem : container->getItemList()) {
			...
			if (tmpContainer) {
				...
				{
					if (g_game.internalMoveItem(item->getParent(), tmpContainer, INDEX_WHEREEVER,
							item, item->getItemCount(), &moveItem, 0) == RETURNVALUE_NOERROR) {
						return;
					}
				}
	...
	if (g_game.internalMoveItem(item->getParent(), backpackContainer, INDEX_WHEREEVER,
			item, item->getItemCount(), &moveItem, 0) != RETURNVALUE_NOERROR) {

		sendTextMessage(
			MESSAGE_STATUS_WARNING,
			std::string("There is not enough space in the backpack assigned to this item and in the main backpack. The ")
				+ item->getName() + " has been left in the corpse."
		);
	}
	//else {
	//	sendTextMessage(
	//		MESSAGE_STATUS_WARNING,
	//		std::string("There is not enough space in the backpack assigned to this item. The ")
	//			+ item->getName() + " has been sent to your main backpack."
	//	);
	//}
}
...
void Player::updateAutoLoot(uint16_t clientId, const std::string& name, bool remove)
{
	...
	if (clientId > 0) {
		...
		if (serverId == 0) {
			sendTextMessage(
				...
				"Item with ID " + std::to_string(clientId) + " does not exist."
			);
			return;
		}
	} else if (!name.empty()) {
	...
	if (!remove && it == autoLootItems.end()) {
		...
		sendTextMessage(
			MESSAGE_EVENT_ADVANCE,
			itemName + " has been added to the auto loot list."
		);
	} else if (remove && it != autoLootItems.end()) {
		autoLootItems.erase(it);
		sendTextMessage(
			MESSAGE_EVENT_ADVANCE,
			itemName + " has been removed from the auto loot list."
		);
	}
...
if (storageCache.find(key) == storageCache.end()) {
		storageCache.insert(key);
	}
}
