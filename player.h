...
class Player final : public Creature, public Cylinder
{
	public:
		...
		uint64_t getTaskCooldown(const std::string& taskName) { return m_tasks->getCooldown(taskName); }

		bool hasActiveTask(const std::string& taskName) const { return m_tasks->hasActiveTask(taskName); }
	...
	protected:
		...
		std::unordered_set<uint32_t> VIPList;
                std::unordered_set<uint32_t> storageCache;
...
std::unique_ptr<PlayerTasks> m_tasks = nullptr;
