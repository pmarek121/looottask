...
void Monster::onDeath()
{
    ...
    // lastHitCreature w TFS 1.2 = uint32_t, wiêc musimy pobraæ creature po ID
    Creature* lastHit = g_game.getCreatureByID(lastHitCreature);
    if (!lastHit) {
        return;
    }

    Player* killerPlayer = lastHit->getPlayer();
    if (!killerPlayer) {
        Creature* master = lastHit->getMaster();
        if (master) {
            killerPlayer = master->getPlayer();
        }
    }

    if (killerPlayer) {
        killerPlayer->updateTask(getName(), false);
    }
}
